...
![Training error (left) and test error (right) on CIFAR-10 with 20-layer and 56-layer "plain" networks. The deeper network has higher training error, and thus test error. Similar phenomena on ImageNet is presented in Fig. 4.](page_1012_370_482_246.png)
...
![Residual learning: a building block.](page_180_120_323_246.png)
...
<table>
  ...
  <tr>
    <td colspan="7">7×7, max pool, stride 2</td>
  </tr>
  <tr>
    <td>conv1</td>
    <td>112×112</td>
  ...
  <tr>
    <td>conv2_x</td>
    <td>56×56</td>
    <td>3×3, 64<br>3×3, 64</td>
    <td>x2</td>
    <td>3×3, 64<br>3×3, 64</td>
    <td>x3</td>
    <td>1×1, 64<br>3×3, 64<br>1×1, 256</td>
    <td>x3</td>
    <td>1×1, 64<br>3×3, 64<br>1×1, 256</td>
    <td>x3</td>
    <td>1×1, 64<br>3×3, 64<br>1×1, 256</td>
  ...
  <tr>
    <td>conv3_x</td>
    <td>28×28</td>
    <td>3×3, 128<br>3×3, 128</td>
    <td>x2</td>
    <td>3×3, 128<br>3×3, 128</td>
    <td>x4</td>
    <td>1×1, 128<br>3×3, 128<br>1×1, 512</td>
    <td>x4</td>
    <td>1×1, 128<br>3×3, 128<br>1×1, 512</td>
    <td>x4</td>
    <td>1×1, 128<br>3×3, 128<br>1×1, 512</td>
  ...
  <tr>
    <td>conv4_x</td>
    <td>14×14</td>
    <td>3×3, 256<br>3×3, 256</td>
    <td>x2</td>
    <td>3×3, 256<br>3×3, 256</td>
    <td>x6</td>
    <td>1×1, 256<br>3×3, 256<br>1×1, 1024</td>
    <td>x6</td>
    <td>1×1, 256<br>3×3, 256<br>1×1, 1024</td>
    <td>x23</td>
    <td>1×1, 256<br>3×3, 256<br>1×1, 1024</td>
  ...
  <tr>
    <td>conv5_x</td>
    <td>7×7</td>
    <td>3×3, 512<br>3×3, 512</td>
    <td>x2</td>
    <td>3×3, 512<br>3×3, 512</td>
    <td>x3</td>
    <td>1×1, 512<br>3×3, 512<br>1×1, 2048</td>
    <td>x3</td>
    <td>1×1, 512<br>3×3, 512<br>1×1, 2048</td>
    <td>x3</td>
    <td>1×1, 512<br>3×3, 512<br>1×1, 2048</td>
  ...
  <tr>
    <td></td>
    <td>1×1</td>
    <td colspan="6">average pool, 1000-d fc, softmax</td>
    <td colspan="5"></td>
  </tr>
  <tr>
    <td>FLOPs</td>
...
![Training curves for ImageNet, comparing plain and ResNet networks](page_101_670_495_312.png)
...
\footnotetext{3We have experimented with more training iterations (3×) and still observed the degradation problem, suggesting that this problem cannot be feasibly addressed by simply using more iterations.}
model
...
for increasing dimensions, and all shortcuts are parameterfree (the same as Table 2 and Fig. 4 right); (B) projection shortcuts are used for increasing dimensions, and other
shortcuts are identity; and (C) all shortcuts are projections.
Table 3 shows that all three options are considerably better than the plain counterpart. B is slightly better than A. We
...
by many (thirteen) projection shortcuts. But the small differences among A/B/C indicate that projection shortcuts are
not essential for addressing the degradation problem. So we
do not use option C in the rest of this paper, to reduce memory/time complexity and model sizes. Identity shortcuts are
...
The parameter-free identity shortcuts are particularly important for the bottleneck architectures. If the identity shortcut in Fig. 5 (right) is replaced with projection, one can
...
mAP areo bike bird boat bottle bus car cat chair cow table dog horse mbike person plant sheep sofa train tv
...
for training and the 20k test-dev set for evaluation. The testdev set has no publicly available ground truth and the result
...
on PASCAL VOC 2012 is 10 points higher than the previous state-of-the-art result [6].
...
proposing regions, and the union set of proposals are processed by an ensemble of per-region classifiers. Table 9
...
as that for MS COCO in Table 9. The networks are pretrained on the 1000-class ImageNet classification set, and
...
mAP@.5 in Table 9), we fine-tune this model on the PASCAL VOC sets. The improvements of box refinement, context, and multi-scale testing are also adopted. By doing so

6 http://host.robots.ox.ac.uk:8080/anonymous/3OJ4OJ.html,
...
<table>
  ...
  <tr>
    <td>VGG’s [41]</td>
    <td>VGG-16<br>ResNet-101<br>ResNet-101</td>
    <td>1-crop dense</td>
    <td>33.1 [41]<br>13.3<br>11.7</td>
    <td></td>
    <td></td>
  </tr>
  <tr>
...
58.8% mAP and our ensemble of 3 models has 62.1% mAP on the DET test set (Table 12). <i>This result won the 1st place in the ImageNet detection task in ILSVRC 2015</i>, surpassing the second place by <b>8.5 points</b> (absolute).

<b>C. ImageNet Localization</b>
...
Our localization algorithm is based on the RPN framework of [32] with a few modifications. Unlike the way in [32] that is category-agnostic, our RPN for localization is designed in a per-class form. This RPN ends with two sibling 1×1 convolutional layers for binary classification (<i>cls</i>) and box regression (<i>reg</i>), as in [32]. The <i>cls</i> and <i>reg</i> layers are both in a <i>per-class</i> from, in contrast to [32]. Specifically, the <i>cls</i> layer has a 1000-d output, and each dimension is <i>binary logistic regression</i> for predicting being or not being an object class; the <i>reg</i> layer has a 1000×4-d output consisting of box regressors for 1000 classes. As in [32], our bounding box regression is with reference to multiple translation-invariant “anchor” boxes at each position.
